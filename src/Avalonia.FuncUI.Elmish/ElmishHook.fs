...
type ElmishState<'model, 'msg, 'arg>(mkProgram : unit -> Program<'arg, 'model, 'msg, unit>, arg: 'arg, setModel) =
    ...
    let mutable _model : 'model = Program.init program arg |> fst
    ...
    let setState model dispatch =
        ...
        let changed =
          use _ = Performance.step "Compare model"
          not (System.Collections.Generic.EqualityComparer.Default.Equals(_model, model))
        if changed then
